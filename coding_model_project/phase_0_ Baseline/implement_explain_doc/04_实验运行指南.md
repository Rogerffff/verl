...
**所有评测参数已统一管理在 `src/eval_config.py` 中**，确保 Baseline → SFT → RL 各阶段评测一致性：
...
| `run_timeout` | **30s** | 代码执行超时（适应 CodeContests） |
| `memory_limit_mb` | 1024 | 内存限制 (1GB) |

⚠️ **这些参数在整个实验流程中必须保持一致，否则结果不可比！**
...
3. 确认镜像版本，推荐使用：
   ...
   - v0.9.1-cuda-12.8-pytorch-2.7.0-py312
...
# 克隆你的代码仓库（替换为你的仓库地址）
git clone https://github.com/Rogerffff/verl.git
cd <your-repo>
...
SandboxFusion 是代码沙盒服务，用于安全执行和评测模型生成的代码。有两种启动方式：
- **方式 A：源码安装**（推荐）- 从 GitHub 拉取源码，本地虚拟环境运行
- **方式 B：Docker 部署** - 快速启动，预装所有运行时

### 6.1 方式 A：从源码安装（推荐）

从 GitHub 仓库拉取代码并在本地虚拟环境中运行。

#### 6.1.0 前置要求：安装 Miniconda

SandboxFusion 依赖 Conda 来管理 Python 运行时环境。如果系统没有安装 Conda，需要先安装：

```bash
# 检查是否已安装 conda
which conda || echo "Conda 未安装"

# 如果未安装，执行以下命令安装 Miniconda
cd /tmp
...
# 创建 conda 环境（Python 3.12）用于运行 Sandbox 服务
...
#### 6.1.3 安装 Poetry 和项目依赖

```bash
# 确保在 sandbox-server 环境中
source /opt/conda/bin/activate sandbox-server
...
# 验证安装
python -c "from sandbox.server.server import app; print('Sandbox installed successfully')"
```

#### 6.1.4 【必需】安装 Python 运行时环境

SandboxFusion 使用独立的 `sandbox-runtime` 环境来执行用户代码。**这是运行 Python 代码的必要条件**：

```bash
# 确保 conda 在 PATH 中
export PATH="/opt/conda/bin:$PATH"

# 创建 sandbox-runtime 环境（Python 3.11）
conda create -n sandbox-runtime python=3.11 -y

# 如果需要安装完整的第三方库（numpy, pandas, torch 等），可执行：
# cd runtime/python
# bash install-python-runtime.sh us  # us 参数使用官方源
# cd ../..
```

> **重要说明**：即使只运行标准库代码，也必须创建 `sandbox-runtime` 环境，否则会报错 `Conda root directory not found`。

#### 6.1.5 启动 SandboxFusion 服务

```bash
# 创建 tmux 会话保持服务后台运行
tmux new -s sandbox

# 确保 conda 在 PATH 中并激活 sandbox-server 环境
export PATH="/opt/conda/bin:$PATH"
source /opt/conda/bin/activate sandbox-server

# 启动服务（在 sandbox 仓库目录下）
cd /workspace/sandbox/sandbox
...
| `--port` | 8080 | 服务端口 |
| `--reload` | - | 开发模式，代码修改后自动重载（生产环境不要用）|

**配置文件**：服务默认读取 `sandbox/configs/local.yaml`，可通过环境变量修改：
```bash
export SANDBOX_CONFIG=local  # 读取 local.yaml（开发模式，无隔离）
export SANDBOX_CONFIG=ci     # 读取 ci.yaml（CI 模式，轻量隔离）
```
...
# 使用其他端口
uvicorn sandbox.server.server:app --host 0.0.0.0 --port 9090
```

---

### 6.2 方式 B：使用 Docker 启动（备选）

Docker 方式预装了所有语言运行时，适合快速启动或生产环境。
...
**重要**：在运行评测脚本的环境中（与 sandbox 服务端分开），需要安装 `sandbox-fusion` 客户端 SDK：
...
# 验证安装
python -c "from sandbox_fusion import run_code, submit; print('SDK installed successfully')"
```

**注意**：
- `sandbox-fusion`（PyPI 包）是**客户端 SDK**，用于调用 sandbox 服务
- `sandbox`（源码）是**服务端**，提供 API 服务
- 两者可以在不同的 Python 环境中运行
...
**数据结构说明**：
- `data/manifests/` - 去重后的 problem_id 列表（较小，已包含在 GitHub 仓库中）
- `data/raw/` - 完整原始数据，包含测试用例（较大，在 `.gitignore` 中排除，**需要单独上传**）
...
预期情况：
- `manifests/` 已有完整数据（164 + 200 + 117 条记录）
- `raw/` 目录存在但为空，或缺少 `*_raw.jsonl` 文件

### 7.2 从本地上传 raw 数据

由于 `raw/` 数据较大已被 `.gitignore` 排除，需要从本地单独上传。

在**本地机器**上执行以下命令（替换 `<PORT>` 和 `<HOST>` 为 vast.ai 提供的 SSH 信息）：

```bash
# =============================================================================
# 只上传 raw 目录（不会影响已有的 manifests）
# =============================================================================

# 方式 1：直接上传 raw 目录下的所有 jsonl 文件
scp -P <PORT> /path/to/local/verl/coding_model_project/data/raw/*_raw.jsonl \
    root@<HOST>:/workspace/rlvr_coding/<your-repo>/verl/coding_model_project/data/raw/

# 方式 2：上传整个 raw 目录（如果远程 raw 目录为空）
scp -P <PORT> -r /path/to/local/verl/coding_model_project/data/raw \
    root@<HOST>:/workspace/rlvr_coding/<your-repo>/verl/coding_model_project/data/

# 方式 3：先打包再上传（网络不稳定时推荐）
# 本地打包 raw 目录
cd /path/to/local/verl/coding_model_project/data
tar czf raw.tar.gz raw/

# 上传压缩包
scp -P <PORT> raw.tar.gz root@<HOST>:/workspace/rlvr_coding/<your-repo>/verl/coding_model_project/data/

# 在远程实例上解压（会覆盖 raw 目录）
ssh -p <PORT> root@<HOST> "cd /workspace/rlvr_coding/<your-repo>/verl/coding_model_project/data && tar xzf raw.tar.gz && rm raw.tar.gz"
```

**示例**（假设 vast.ai SSH 信息为 `ssh -p 12345 root@ssh.vast.ai`）：
```bash
# 本地执行 - 只上传 raw 数据文件
scp -P 12345 ~/Desktop/verl/verl/coding_model_project/data/raw/*_raw.jsonl \
    root@ssh.vast.ai:/workspace/rlvr_coding/verl/verl/coding_model_project/data/raw/
```

**关于覆盖行为**：
- `scp` 复制**同名文件会覆盖**，但**不会删除**目标目录中的其他文件
- 只上传 `raw/` 不会影响 `manifests/` 目录
- 如果 `raw/` 中已有部分文件，同名文件会被覆盖，其他文件保留
...
=== Raw Data ===
   164 data/raw/humaneval_raw.jsonl
   200 data/raw/mbpp_reg_raw.jsonl
   117 data/raw/codecontests_valid_raw.jsonl
   ...（可能还有其他数据集）
```

### 7.4 备选：在服务器上重新获取数据

如果本地也没有数据文件，可以在服务器上运行数据治理脚本重新获取：
...
**注意**：这种方式需要从 HuggingFace 下载数据，可能较慢。建议优先使用本地上传方式。
...
提供两个版本的一键运行脚本：
- **版本 A**：源码安装版（推荐）- 从 GitHub 拉取 sandbox 源码运行
- **版本 B**：Docker 版 - 使用预构建的 Docker 镜像

### 11.1 版本 A：源码安装版（推荐）

将以下脚本保存为 `run_phase0_eval_source.sh`：
...
echo "============================================================"
echo "   Phase 0 Baseline Evaluation (Source Install)"
echo "============================================================"
echo ""
...
SANDBOX_DIR="/workspace/sandbox/sandbox"
OUTPUT_DIR="${WORK_DIR}/outputs/phase0_$(date +%Y%m%d_%H%M%S)"

# =============================================================================
# 1. 安装 SandboxFusion（如果未安装）
# =============================================================================
echo "[1/6] Setting up SandboxFusion from source..."
...
if [ ! -d "${SANDBOX_DIR}" ]; then
    ...
    echo "  SandboxFusion installed successfully!"
else
    echo "  SandboxFusion already installed at ${SANDBOX_DIR}"
fi
...
# =============================================================================
echo ""
echo "[2/6] Starting SandboxFusion server..."
...
if curl -s http://localhost:${SANDBOX_PORT}/health > /dev/null 2>&1; then
    echo "  SandboxFusion is already running"
else
    cd ${SANDBOX_DIR}
    nohup uvicorn sandbox.server.server:app --host 0.0.0.0 --port ${SANDBOX_PORT} \
...
# =============================================================================
echo ""
echo "[4/6] Installing SDK and verifying services..."

# 安装 sandbox-fusion SDK
pip install sandbox-fusion -q
...
# =============================================================================
echo ""
echo "[5/6] Running Phase 0 evaluation..."
...
将以下脚本保存为 `run_phase0_eval_docker.sh`：
...
echo "============================================================"
echo "   Phase 0 Baseline Evaluation (Docker)"
echo "============================================================"
echo ""
...
# =============================================================================
echo "[1/5] Starting SandboxFusion (Docker)..."
...
```bash
# 选择一个版本的脚本，修改其中的 WORK_DIR 为你的实际路径

# 版本 A：源码安装版（推荐）
chmod +x run_phase0_eval_source.sh
./run_phase0_eval_source.sh

# 或者版本 B：Docker 版
chmod +x run_phase0_eval_docker.sh
./run_phase0_eval_docker.sh
```
