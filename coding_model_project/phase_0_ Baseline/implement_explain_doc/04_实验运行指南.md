...
#### 6.1.4 （可选）安装 Python 运行时环境

如果需要执行包含第三方库的 Python 代码，可以安装完整运行时：

```bash
# 安装 Python 运行时（包含 numpy, pandas, torch 等常用库）
cd runtime/python
bash install-python-runtime.sh
cd ../..
```

这会创建名为 `sandbox-runtime` 的 conda 环境，包含 100+ 常用 Python 库。

**注意**：对于基础评测（HumanEval、MBPP），通常不需要这一步，因为测试用例只使用标准库。
