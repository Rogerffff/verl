你问得非常对：**`rratio = -0.3 + 1.3*pass_ratio` 以及“错误类型 piecewise 惩罚”的具体数值，不能拍脑袋**。好消息是——这两块其实都有明确“出处”，而且你 PDF 里那组系数几乎可以确定是从某篇经典工作里“搬过来”的。

下面我把 **Dense-linear 里三项（rratio / rerr / rlen）各自的依据**讲清楚，并给你一套**更“论文对齐”的实现方式 + 计算例子**。
