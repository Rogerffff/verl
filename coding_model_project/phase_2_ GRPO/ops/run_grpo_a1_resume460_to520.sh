...
TOTAL_TRAINING_STEPS=${TOTAL_TRAINING_STEPS:-520}
TEST_FREQ=${TEST_FREQ:-20}
SAVE_FREQ=${SAVE_FREQ:-490}
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-"grpo_a1_formal_observe_lb96_multi6_resume460_to520_seed${SEED}"}
...
resolve_resume_from_path() {
    ...
    local candidate
    local -a candidates=(
        "/workspace/verl/checkpoints/rlvr_coding_model/grpo_a1_formal_observe_lb64_multi4_resume400_to460_seed0/global_step_460"
        "${PHASE2_DIR%/coding_model_project/phase_2_ GRPO}/checkpoints/rlvr_coding_model/grpo_a1_formal_observe_lb64_multi4_resume400_to460_seed0/global_step_460"
    )
    ...
    echo "ERROR: Could not resolve a valid step460 checkpoint. Set RESUME_FROM_PATH explicitly." >&2
    exit 1
}
