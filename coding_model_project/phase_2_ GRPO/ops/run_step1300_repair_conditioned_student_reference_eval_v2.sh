...
ARTIFACT_TAG=${ARTIFACT_TAG:-step1300_repair_cond_v2}
DATA_ROOT=${DATA_ROOT:-"${PHASE2_DIR}/sft_repair_data/${ARTIFACT_TAG}"}
...
PILOT_QUEUE_JSONL="${DATA_ROOT}/pilot_teacher_requests_${ARTIFACT_TAG}.jsonl"
PILOT_QUEUE_SUMMARY_JSON="${DATA_ROOT}/pilot_teacher_requests_${ARTIFACT_TAG}.summary.json"

TRAIN_RAW=${TRAIN_RAW:-"${PROJECT_ROOT}/data/raw/codecontests_train_wo_valid_big_raw.jsonl"}
CKPT_DIR=${CKPT_DIR:-"${ROOT}/checkpoints/rlvr_coding_model/grpo_a1_curriculum_step1200_followup_u6a2b4c3d1_to1400_lb240_seed0_vastai3/global_step_1300"}
...
build_pilot_queue() {
    log "BUILD_PILOT_QUEUE"
    "$PYTHON_BIN" "${SCRIPTS_DIR}/build_step1300_repair_pilot_queue_v2.py" \
        --student_references "$STUDENT_REFS_JSONL" \
        --output "$PILOT_QUEUE_JSONL" \
        --summary_out "$PILOT_QUEUE_SUMMARY_JSON"
}
...
main() {
    ...
    build_student_references
    build_pilot_queue
    log "DONE step1300 repair-conditioned Stage B + pilot"
}
